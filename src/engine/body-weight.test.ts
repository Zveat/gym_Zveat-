...
describe('скорость изменения веса', () => {
  ...
  /**
   * ЭКРАН ВЛАДЕЛЬЦА, 18.09.2026: 1 августа 81 кг, 15 сентября 85,2, сегодня
   * 86,1. Приложение говорило «Пока нечего оценивать, между взвешиваниями 3
   * дня» — сидя на 48 днях и +5,1 кг в собственной базе.
   *
   * Причина была в том, что кандидатов рассматривалось ровно два: самый
   * старый в окне и предпоследний замер. Здесь это ОДНА И ТА ЖЕ запись за 15
   * сентября, и до первого августа очередь не доходила никогда.
   */
  it('когда свежий период слишком короткий, берёт замер постарше, а не сдаётся', () => {
    const rate = bodyWeightRate(
      [log('2026-08-01', 81), log('2026-09-15', 85.2), log('2026-09-18', 86.1)],
      new Date('2026-09-18T14:41:00'),
    );
    expect(rate?.from.date).toBe('2026-08-01');
    expect(rate?.spanDays).toBe(48);
    // +5,1 кг за 48 дней — это 0,74 кг в неделю, и это уже есть что сказать.
    expect(rate?.perWeek).toBe(0.74);
  });

  it('и вердикт на этих данных перестаёт быть «нечего оценивать»', () => {
    ...
    expect(v.detail).toContain('за 48 дней');
  });

  it('свежий период всё равно в приоритете, когда он достаточно длинный', () => {
    // 20.08 годится (27 дней) — значит первое августа не берём, хотя оно старше.
    const rate = bodyWeightRate(
      [log('2026-08-01', 81), log('2026-08-20', 83), log('2026-09-16', 84)],
      NOW,
    );
    expect(rate?.from.date).toBe('2026-08-20');
  });
